...
describe("TaskForm", () => {
  ...
  it("previews and applies AI task draft suggestions", async () => {
    ...
    expect(
      await screen.findByText(/ai refined lab report/i),
    ).toBeInTheDocument();

    fireEvent.click(
      screen.getByRole("button", { name: /apply suggestions/i }),
    );

    expect(screen.getByLabelText(/task name/i)).toHaveValue(
      "AI refined lab report",
    );
  ...
  it("shows the specific AI draft error message from the route", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /apply suggestions/i }),
