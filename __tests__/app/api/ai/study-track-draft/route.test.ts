...
const mockGetUserStudyPreferences = getUserStudyPreferences as jest.MockedFunction<typeof getUserStudyPreferences>;
const mockGetUserAvailability = getUserAvailability as jest.MockedFunction<typeof getUserAvailability>;
const mockGetUserBehaviorProfile = getUserBehaviorProfile as jest.MockedFunction<typeof getUserBehaviorProfile>;
const mockListTaskAttachments = listTaskAttachments as jest.MockedFunction<typeof listTaskAttachments>;
const mockGenerateStudyTrackDraft = generateStudyTrackDraft as jest.MockedFunction<typeof generateStudyTrackDraft>;
...
type GetUserStudyPreferencesResolved = Awaited<ReturnType<typeof getUserStudyPreferences>>;
type GetUserAvailabilityResolved = Awaited<ReturnType<typeof getUserAvailability>>;
type GetUserBehaviorProfileResolved = Awaited<ReturnType<typeof getUserBehaviorProfile>>;
type ListTaskAttachmentsResolved = Awaited<ReturnType<typeof listTaskAttachments>>;
type GenerateStudyTrackDraftResolved = Awaited<ReturnType<typeof generateStudyTrackDraft>>;

interface StudyTrackResponseBody {
  draft?: {
    tracks: Array<{
      title: string;
      start_date: string;
...
describe("POST /api/ai/study-track-draft", () => {
  beforeEach(() => {
    ...
    mockGetTaskById.mockResolvedValue({
      ...
      task_priority: 4,
    } as unknown as GetTaskByIdResolved);

    mockGetUserStudyPreferences.mockResolvedValue({
      focus_minutes: 25,
      break_minutes: 5,
      total_pomodoros: 2,
      total_minutes: 60,
    } as GetUserStudyPreferencesResolved);

    mockGetUserAvailability.mockResolvedValue([
      { day_of_week: 1, start_time: "15:00", end_time: "17:00" },
    ] as GetUserAvailabilityResolved);

    mockGetUserBehaviorProfile.mockResolvedValue(null as GetUserBehaviorProfileResolved);
    ...
    ] as ListTaskAttachmentsResolved);

    mockGenerateStudyTrackDraft.mockResolvedValue({
      tracks: [
        {
      ...
      skippedAttachments: [],
    } as GenerateStudyTrackDraftResolved);
  });
  ...
  it("asks Gemini for a task-linked study track draft", async () => {
    ...
    const body = await response.json() as StudyTrackResponseBody;

    expect(response.status).toBe(200);
    expect(mockGenerateStudyTrackDraft).toHaveBeenCalledWith(
  ...
  it("asks Gemini for a standalone study-session draft without taskId", async () => {
    ...
    expect(mockListTaskAttachments).not.toHaveBeenCalled();
    expect(mockGenerateStudyTrackDraft).toHaveBeenCalledWith(
      expect.objectContaining({
        task: null,
        session: expect.objectContaining({
  ...
  it("propagates prompt-injection safety errors as JSON", async () => {
    mockGenerateStudyTrackDraft.mockRejectedValue(
